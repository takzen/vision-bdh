...
# ==============================================================================
# Training Script for EfficientNet-B0 on CIFAR-100
# ==============================================================================
...
def main():
    ...
    CHECKPOINT_DIR = "./checkpoints_efficientnet_cifar100"
    LOG_FILE = os.path.join(CHECKPOINT_DIR, "metrics_efficientnet_cifar100.csv")

    os.makedirs(CHECKPOINT_DIR, exist_ok=True)

    print("=" * 60)
    print("     Training EfficientNet-B0 on CIFAR-100 (Baseline)")
    print("=" * 60)
    ...
    # --- Model ---
    # Load the EfficientNet-B0 model, training from scratch (weights=None) for 100 classes.
    model = efficientnet_b0(weights=None, num_classes=100).to(DEVICE)
    
    optimizer = AdamW(model.parameters(), lr=INITIAL_LR, weight_decay=0.05)

    num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"EfficientNet-B0 created with {num_params / 1e6:.2f}M trainable parameters.")
    ...
    final_model_path = os.path.join(CHECKPOINT_DIR, 'final_model_efficientnet_cifar100.pth')
