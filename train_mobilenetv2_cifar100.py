...
# ==============================================================================
# Training Script for MobileNetV2 on CIFAR-100
# ==============================================================================
...
def main():
    ...
    CHECKPOINT_DIR = "./checkpoints_mobilenetv2_cifar100"
    LOG_FILE = os.path.join(CHECKPOINT_DIR, "metrics_mobilenetv2_cifar100.csv")

    os.makedirs(CHECKPOINT_DIR, exist_ok=True)

    print("=" * 60)
    print("     Training MobileNetV2 on CIFAR-100 (Baseline)")
    print("=" * 60)
    ...
    # --- Model ---
    # Load the MobileNetV2 model, training from scratch (weights=None) for 100 classes.
    model = mobilenet_v2(weights=None, num_classes=100).to(DEVICE)
    
    optimizer = AdamW(model.parameters(), lr=INITIAL_LR, weight_decay=0.05)

    num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"MobileNetV2 created with {num_params / 1e6:.2f}M trainable parameters.")
    ...
    # Note: The analysis script will pick the best checkpoint based on validation accuracy.
    # This final evaluation is just for a quick check on the last epoch's model.
    print("\n" + "=" * 60)
    ...
    final_model_path = os.path.join(CHECKPOINT_DIR, 'final_model_mobilenetv2_cifar100.pth')
